...
function drawBackground() {
    ...
    ctx.drawImage(sky.image, 
         0, 0, sky.image.width, sky.image.height,
    ...
    ctx.drawImage(mountains.image, 
         ...
         0, 0, imageEnd, canvas.height);
    // For mountains wrapping
    if (imageEnd < canvas.width) {
       ctx.drawImage(mountains.image, 
            0, 0, canvas.width-imageEnd, mountains.image.height,
            imageEnd, 0, canvas.width-imageEnd, canvas.height);
    }
    // ctx.drawImage(mountains.image, 0, 0, canvas.width-mountains.x, canvas.height);
    // ctx.drawImage(mountains.image, canvas.width+mountains.x, 0, canvas.width, canvas.height);
...
function drawPlayerShadows() {
    ...
    ctx.beginPath();
    ctx.ellipse(player.x + player.width/2 + player.riderXOffset, player.y + player.height + player.riderYOffset + 54, 20, 4.2, 0, 0, Math.PI * 2);
    ctx.fill();
}
...
function drawPlayer() {
    ...
    ctx.drawImage(
        ...
        player.x, player.height-player.boardSprite.height + player.y, player.width, player.height
    );
    ...
    ctx.drawImage(
        ...
        player.height-player.boardSprite.height + player.y + player.riderYOffset, player.width, player.height
    );
}
...
function isPlayerCollision(spriteA) {
    return (
        ...
        player.y + player.height > (spriteA.y + spriteA.height/3)
    );
}
...
function menu() {  
    ...
    ctx.font = "24px Arial"; // Set the font size and family
    ctx.fillStyle = "#000"; // Set the text color
    ctx.fillText("Choose your board:", 10, 180);
...
function update() {
    ...
    if (!player.isDead) {
       ...
       if (player.speed < speed)
          player.speed = speed;
       if (audio.playbackRate != (player.speed / 12 + 0.5).toFixed(1)) {
          audio.playbackRate = (player.speed / 12 + 0.5).toFixed(1); 
